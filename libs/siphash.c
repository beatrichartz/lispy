...
static inline void u64to8_le(uint8_t *p, uint64_t v) {
    U32TO8_LE(p, v.lo);
    U32TO8_LE(p + 4, v.hi);
}
...
static inline uint64_t *rotl64_to(uint64_t *v, unsigned int s) {
...
static inline uint64_t *rotl64_swap(uint64_t *v) {
    uint32_t t = v->lo;
    v->lo = v->hi;
    v->hi = t;
    return v;
...
static inline uint64_t *add64_to(uint64_t *v, const uint64_t s) {
    v->lo += s.lo;
    v->hi += s.hi;
    if (v->lo < s.lo)
        v->hi++;
...
static inline uint64_t *xor64_to(uint64_t *v, const uint64_t s) {
    v->lo ^= s.lo;
    v->hi ^= s.hi;
    return v;
}
...
static const char sip_init_state_bin[] = "uespemos"
                                         "modnarod"
                                         "arenegyl"
                                         "setybdet";
...
static const sip_interface sip_methods = {int_sip_init, int_sip_update,
                                          int_sip_final};
...
static void int_sip_init(sip_state *state, uint8_t key[16]) {
    uint64_t k0, k1;
    ...
    state->v[0] = k0;
    XOR64_TO(state->v[0], sip_init_state[0]);
    state->v[1] = k1;
    XOR64_TO(state->v[1], sip_init_state[1]);
    state->v[2] = k0;
    XOR64_TO(state->v[2], sip_init_state[2]);
    state->v[3] = k1;
    XOR64_TO(state->v[3], sip_init_state[3]);
}

static inline void int_sip_round(sip_state *state, int n) {
    int i;
...
static inline void int_sip_update_block(sip_state *state, uint64_t m) {
    XOR64_TO(state->v[3], m);
    int_sip_round(state, state->c);
    XOR64_TO(state->v[0], m);
}

static inline void int_sip_pre_update(sip_state *state, uint8_t **pdata,
                                      size_t *plen) {
    int to_read;
    uint64_t m;
...
static inline void int_sip_post_update(sip_state *state, uint8_t *data,
                                       size_t len) {
...
static inline void int_sip_pad_final_block(sip_state *state) {
    int i;
...
int sip_hash_update(sip_hash *h, uint8_t *msg, size_t len) {
...
int sip_hash_final(sip_hash *h, uint8_t **digest, size_t *len) {
    ...
    if (!(ret = (uint8_t *)malloc(sizeof(uint64_t))))
        return 0;
...
int sip_hash_digest(sip_hash *h, uint8_t *data, size_t data_len,
                    uint8_t **digest, size_t *digest_len) {
    if (!sip_hash_update(h, data, data_len))
        return 0;
    return sip_hash_final(h, digest, digest_len);
}

int sip_hash_digest_integer(sip_hash *h, uint8_t *data, size_t data_len,
                            uint64_t *digest) {
    if (!sip_hash_update(h, data, data_len))
        return 0;
...
uint64_t sip_hash24(uint8_t key[16], uint8_t *data, size_t len) {
    ...
    v0 = k0;
    XOR64_TO(v0, sip_init_state[0]);
    v1 = k1;
    XOR64_TO(v1, sip_init_state[1]);
    v2 = k0;
    XOR64_TO(v2, sip_init_state[2]);
    v3 = k1;
    XOR64_TO(v3, sip_init_state[3]);
