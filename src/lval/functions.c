...
lval *builtin_head(lval *v) {
    LISPY_ASSERT(v, v->count == 1, "Function 'head' passed too many arguments");

    LISPY_ASSERT(v, v->cell[0]->type == LVAL_QEXPR,
                 "Function 'head' passed an incorrect type");

    LISPY_ASSERT(v, v->cell[0]->count > 0,
...
lval *builtin_cons(lval *v) {
    LISPY_ASSERT(v, v->count > 1, "Function 'cons' passed too few arguments");

    LISPY_ASSERT(v, v->count < 3, "Function 'cons' passed too many arguments");

    LISPY_ASSERT(v, v->cell[1]->type == LVAL_QEXPR,
...
lval *builtin_init(lval *v) {
    LISPY_ASSERT(v, v->count == 1, "Function 'init' passed too many arguments");
...
lval *builtin_tail(lval *v) {
    LISPY_ASSERT(v, v->count == 1, "Function 'tail' passed too many arguments");
...
lval *builtin_eval(lval *v) {
    LISPY_ASSERT(v, v->count == 1, "Function 'eval' passed too many arguments");
...
lval *builtin_join(lval *v) {
    for (int i = 0; i < v->count; i++) {
        LISPY_ASSERT(v, v->cell[i]->type == LVAL_QEXPR,
...
lval *lval_join(lval *a, lval *b) {
    ...
    memmove(&a->cell[a->count - b->count], &b->cell[0],
            sizeof(lval *) * b->count);
...
lval *builtin_len(lval *v) {
    LISPY_ASSERT(v, v->count == 1, "Function 'len' passed too many arguments");
