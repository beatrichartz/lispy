...
void print_run_start(runner *r, suite *s) {
    int size =
        s->focused_tests > 0 ? s->focused_tests : (s->size - s->crossed_tests);
    printf("\n%s**************************************\
 RUNNING %4.1d TESTS\
 ***************************************%s\n\n",
           KCYN, size, RESET);
...
void print_run_pending(runner *r, suite *s) {
    ...
    for (int i = 0; i < s->size; i++) {
        test *t = s->tests[i];
        if (t->outcome == TEST_FAILED) {
            continue;
        }
        if (t->outcome == TEST_PASSED) {
            continue;
        }
        if (s->focused_tests > 0 && !t->focused) {
            continue;
        }
        if (s->crossed_tests > 0 && t->crossed) {
            continue;
        }
        printf("%s%s is pending: %s%s\n", KYLW, t->func_name,
               t->pending_message, RESET);
    }
    printf("\n");
}
...
void print_run_failures(runner *r, suite *s) {
    ...
    for (int i = 0; i < s->size; i++) {
        ...
        if (t->outcome == TEST_PENDING) {
            continue;
        }
        printf("%sfailed assertion: %s\n%s\n%s:%d%s\n\n", KRED,
               t->failure_message, t->func_name, t->file_name, t->line_number,
               RESET);
    }
    printf("\n");
}
...
int should_run_test(suite *s, test *t) {
    return (((s->focused_tests > 0 && t->focused) || (s->focused_tests == 0)) &&
            !t->crossed);
}
